...
仓库提供同级封面图技能（`../wechat-cover/`），调用 Gemini Image API（或兼容的第三方网关）生成公众号封面图（2.35:1 Notion 插画风格），并复用当前技能的 `scripts/generate.py`。
...
完整提示词模板和工作流详见 [`../wechat-cover/SKILL.md`](../wechat-cover/SKILL.md)。
