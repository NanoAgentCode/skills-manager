...
def add_title(doc, title_text):
    ...
    p.paragraph_format.space_before = Pt(0)
    # Use paragraph spacing instead of an empty paragraph so the document
    # remains structurally clean while preserving one-line visual separation.
...
def add_page_number(doc, skip_first=False):
    ...
    # 启用奇偶页不同
    # Use python-docx's schema-aware setter so the element is inserted in the
    # correct CT_Settings order. Appending raw XML makes OfficeCLI validation
    # fail with an unexpected-child error.
