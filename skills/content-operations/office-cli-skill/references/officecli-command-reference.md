...
This repository does not bundle the executable. Resolve it from `OFFICECLI_BIN` or `PATH` and verify with `officecli --version`. Installing or downloading the external tool requires user approval; follow the vendor's current installation documentation rather than executing an unreviewed remote script from this reference.
