...
License: MIT. This local copy is integrated under `skills/knowledge-visualization/drawio-skill/`.

Compatibility: requires the draw.io desktop app CLI on PATH for local export on macOS, Linux, or Windows. The self-check step requires a vision-enabled model and is skipped gracefully when unavailable. Optional auto-layout through `scripts/autolayout.py` needs Graphviz (`dot`).
...
| Card-card overlap | Any card/container visually covers another card/container, even partially | Move one card to a free grid cell or enlarge the canvas; do not rely on z-order to hide the collision |
| Text-card overlap | A title, subtitle, note, legend, or label is under a card/container or arrowhead | Reserve more vertical/horizontal whitespace and reroute arrows around the text zone |
| Text overflow | Text extends beyond its card/container boundary | Increase card width/height, split the label into shorter lines with `&#xa;`, or reduce only secondary text size |
