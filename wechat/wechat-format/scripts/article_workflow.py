...
def require_ai_config(config: dict) -> dict:
    ...
    if not url or not key or not model:
        raise SystemExit(
            "Missing AI config for structured/enhanced generation. Fill wechat/wechat-format/config.json -> ai "
...
def run_cover_generation(prompt_path: Path, cover_path: Path, env: dict[str, str]) -> None:
    ...
    if not config_path.exists():
        raise SystemExit(
            "Cover generation requested, but wechat/wechat-format/wechat-cover/config.json is missing."
        )
    cmd = [
        sys.executable,
